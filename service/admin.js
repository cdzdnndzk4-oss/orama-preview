...
$('#new').onclick=()=>{current=null;photos={};$('#product').reset();$('#product').elements.id.disabled=false;$('#title').textContent='Νέο προϊόν';renderPhotos();};
function edit(p){current=p;photos=Object.fromEntries((p.photos||[]).map(x=>[x.view,x]));const f=$('#product').elements;
  ...
  f.selection.checked=p.selection;f.published.checked=p.published;f.id.disabled=true;$('#title').textContent=p.brand+' '+p.model;renderPhotos();window.scrollTo(0,0);
}
